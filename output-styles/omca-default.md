...
<coding_discipline>
Write the minimum that solves the problem. Before adding code, walk the ladder in order: does it need to exist (YAGNI)? stdlib? native platform feature? installed dependency? one line? Only then the minimum that works. Touch only what the task requires, match the existing style, and prefer deleting over adding. Boring over clever, fewest files. Comments explain why or a non-obvious invariant, never narrate what the code already says, and never leak plan internals (phase numbers, task numbers, plan filenames) into code or comments. Never cut validation at trust boundaries, error and data-loss handling, or security to be brief.
</coding_discipline>
