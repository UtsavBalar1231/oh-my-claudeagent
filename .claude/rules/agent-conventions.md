...
- **Model field**: declare a tier alias, which is what every OMCA agent does, so an agent tracks whatever the platform's current model is for that tier and the frontmatter can never go stale. The roster declares two: `opus` for every agent the plugin spawns, `fable` for oracle. Full IDs (`claude-opus-5`, `claude-sonnet-5`, `claude-fable-5`) still work and remain valid as a per-invocation `Agent(..., model=...)` override, but do not put one in frontmatter: a pinned ID is a recurring maintenance surface that has to be swept across every agent file each time a generation ships. Other aliases: `opusplan` uses Opus in plan mode and Sonnet for execution; `best` resolves to the most capable available model. `sonnet` and `haiku` are off the roster but remain supported override models, so `Agent(..., model="sonnet")` still works; just don't assign either as an agent's default `model:`.

  Because one tier now covers the whole roster minus oracle, `model:` no longer distinguishes a searcher from a planner. `effort:` is the field that does, so a new agent's effort level is the deliberate choice, not its model.
  ...
  - **Third-party providers**: aliases resolve per provider, so on some of them a tier lands on an older generation than on the Anthropic API. `opus` resolves to Opus 4.6 on Microsoft Foundry, and is Opus 5 on the Anthropic API, Claude Platform on AWS, Amazon Bedrock, and Google Cloud's Agent Platform. With `opus` on every agent but oracle, that one provider gap now reaches the whole roster rather than a subset of it. It also costs an effort level there: Opus 4.6 supports `low`, `medium`, `high`, and `max` but not `xhigh` (`claude-code-docs/docs/model-config.md`, "Adjust effort level"), and the platform falls back to the highest supported level at or below the one set, so an `xhigh` agent runs at `high` on Foundry. Users who need a specific generation on such a provider set it in their own settings (`ANTHROPIC_DEFAULT_OPUS_MODEL` and siblings, which take a full provider model ID, never an alias) rather than in plugin agent frontmatter.
