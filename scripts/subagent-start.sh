...
case "${AGENT_TYPE}" in
	*explore*|*librarian*|*oracle*)
		...
		CONTEXT_PARTS+="[EXTERNAL PATH ACCESS] The Read tool is scoped to the project root for subagents. For files outside the project root, use Bash(cat /path/to/file) instead. If Bash is also restricted (e.g., inherited plan mode), report the limitation and proceed with project-internal files."
		;;
	*) ;;
esac
