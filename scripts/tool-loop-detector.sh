...
if [[ -f "${STATE_FILE}" ]]; then
	read -r PREV_SIG PREV_COUNT < <(jq -r '[(.signature // ""), (.count // 0)] | @tsv' "${STATE_FILE}" 2>/dev/null)
...
if jq -n --arg sig "${SIG}" --argjson count "${NEW_COUNT}" '{signature: $sig, count: $count}' > "${TMP}" 2>/dev/null; then
