...
if echo "${CLEANED}" | grep -q '^## ' 2>/dev/null; then
	# 150 lines — structured limit (## sections present); headroom keeps sections intact.
	LIMIT=150
else
	# 100 lines — unstructured limit; prose-only context needs less to orient the model.
	LIMIT=100
fi
